...
fn assert_artifact_fixture(name: &str, mode: ParseMode, args: &[Value]) {
    let source_path = fixture_path(name, "m");
    let source = fs::read_to_string(&source_path).expect("fixture source");
    let expected = fs::read_to_string(fixture_path(name, "golden")).expect("fixture golden");
    ...
    let artifact = encode_bytecode_module(&bytecode);
    let decoded = decode_bytecode_module(&artifact).expect("decode artifact");
    let result = match unit.kind {
        CompilationUnitKind::Script => {
            execute_script_bytecode_module(&decoded, source_path.display().to_string())
        }
...
fn assert_bundle_fixture(name: &str, mode: ParseMode, args: &[Value]) {
    let source_path = fixture_path(name, "m");
    let source = fs::read_to_string(&source_path).expect("fixture source");
    let expected = fs::read_to_string(fixture_path(name, "golden")).expect("fixture golden");
    ...
    let bundle = BytecodeBundle {
        root_source_path: source_path.display().to_string(),
        root_module: rewrite_bytecode_bundle_targets(&bytecode, &path_to_module_id),
        dependency_modules,
    };
    let result = match unit.kind {
...
#[test]
fn builtin_switch_artifact_fixture_matches_golden() {
    assert_artifact_fixture("builtin_switch", ParseMode::Script, &[]);
}

#[test]
fn function_file_nested_capture_artifact_fixture_matches_golden() {
    assert_artifact_fixture(
        "function_file_nested_capture",
        ParseMode::AutoDetect,
        &[Value::Scalar(4.0)],
    );
...
#[test]
fn external_resolution_artifact_fixture_matches_golden() {
    assert_artifact_fixture("external_resolution", ParseMode::Script, &[]);
}

#[test]
fn global_persistent_runtime_artifact_fixture_matches_golden() {
    assert_artifact_fixture("global_persistent_runtime", ParseMode::Script, &[]);
}

#[test]
fn undefined_struct_root_assignment_artifact_fixture_matches_golden() {
    assert_artifact_fixture(
        "undefined_struct_root_assignment",
        ParseMode::AutoDetect,
        &[],
...
#[test]
fn implicit_root_growth_artifact_fixture_matches_golden() {
    assert_artifact_fixture("implicit_root_growth", ParseMode::Script, &[]);
}

#[test]
fn comma_separated_cells_artifact_fixture_matches_golden() {
    assert_artifact_fixture("comma_separated_cells", ParseMode::Script, &[]);
}

#[test]
fn external_resolution_bundle_fixture_matches_golden() {
    assert_bundle_fixture("external_resolution", ParseMode::Script, &[]);
}

#[test]
fn external_function_handles_bundle_fixture_matches_golden() {
    assert_bundle_fixture("external_function_handles", ParseMode::Script, &[]);
}

#[test]
fn multi_output_external_calls_bundle_fixture_matches_golden() {
    assert_bundle_fixture("multi_output_external_calls", ParseMode::Script, &[]);
}

#[test]
fn external_global_persistent_runtime_bundle_fixture_matches_golden() {
    assert_bundle_fixture("external_global_persistent_runtime", ParseMode::Script, &[]);
}
