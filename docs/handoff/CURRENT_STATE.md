...
- the repo now has a first real `classdef` baseline instead of only the old `MException` compatibility shim: parser autodetect recognizes class files, semantics/HIR record class metadata, resolver supports plain class files plus `@ClassName` folder definitions and separate method files, runtime values now include scalar value and handle objects, interpreter and bytecode execution both support constructor calls plus dot-call and function-form method dispatch for the implemented subset, and `class(...)` / `isa(...)` now report user class names plus builtin `handle` compatibility for handle classes
- the current classdef slice is intentionally narrow: only one class per file, only public instance properties/methods, only scalar objects, only builtin `handle` inheritance, only the current default-expression subset, and no attributes/events/enumeration/static/user-defined inheritance/object arrays/metaclass surface yet
